...
The iterative consensus refinement aims to reduce scaffolding reference bias and improve consensus accuracy by repeatedly mapping reads to the generated consensus, calling variants, and applying those variants to update the consensus genome so it better reflects the true sample sequence. This refinement process performs up to four iterative cycles (default: two). In each iteration, reads are mapped to the consensus using BWA-MEM2 \citep{Vasimuddin2019-rb} or Bowtie2 \citep{Langmead2019-wx}, followed by true minor variant calling with BCFtools \citep{Danecek2021-je} or iVar \citep{Grubaugh2019-xd}. Benchmarking by Bassano et al. \citep{Bassano2022-cl} demonstrated that BCFtools outperformed iVar in precision and recall, although iVar detected more low-frequency variants. Users are therefore advised to choose their variant caller based on whether they wish to prioritize sensitivity or specificity.
