...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.8\textwidth]{Fig/suppl_fig1.png}
...
For plant pathogens, two samples were randomly selected from the SRA after querying for the following viruses: ``\textit{Tobacco mosaic virus}'', ``\textit{Tomato spotted wilt virus}'', ``\textit{Cucumber mosaic virus}'', and ``\textit{Potato virus Y}''. These were also downloaded using nf-core/fetchngs and processed with viralmetagenome using the same parameters (default options with \texttt{--cluster\_method 'mmseqs-cluster'} and \texttt{--skip\_read\_classification}), with the addition of the Virosaurus plant virus database as annotation database (\texttt{--annotation\_db} \href{https://ftp.expasy.org/databases/viralzone/2020_4/virosaurus98_plant-20200330.fas.gz}{\url{https://ftp.expasy.org/databases/viralzone/2020_4/virosaurus98_plant-20200330.fas.gz}}) and RVDB's v29.0 unclustered database as reference pool (\texttt{--reference\_pool} \href{https://rvdb.dbi.udel.edu/download/U-RVDBv29.0.fasta.gz}{\url{https://rvdb.dbi.udel.edu/download/U-RVDBv29.0.fasta.gz}}).
...
\toprule
\textbf{Sample name} & \textbf{Genome 1} & \textbf{Genome 2} & \textbf{Similarity to reference (\%)} & \textbf{Reads (M)} & \textbf{Read \% genome 1} & \textbf{Read \% genome 2} \\
\midrule
