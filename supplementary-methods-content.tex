...
To establish this gradient, we first generated a baseline consensus for each sample using the unclustered RVDB (U-RVDBv29.0) as the reference pool. We then retrieved all \textit{Mammarenavirus lassaense} (taxid: 3052310) sequences from NCBI and calculated their ANI against these baseline consensus genomes using MMseqs2 \citep{Steinegger2017-ci}. From this pool, we selected one scaffolding reference genome at every 2\% ANI interval for each sample (Supplementary Figure 2). The Lassa virus samples exhibited differential coverage between segments (S segment mean ~231X, L segment mean ~91X), allowing us to evaluate scaffolding performance on both complete and fragmented assemblies.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{supplfig2}
    \caption{Jitter-boxplot for scaffolding reference genomes considered in the sample setup for determining the influence of the scaffolding reference. X-axis represents the samples used, y-axis represents the MMseqs ANI similarity towards the consensus genome. The consensus genome that is compared to all the references is the consensus genome generated by nf-core/viralmetagenome when automatic reference selection was applied for the U-RVDB reference library supplied with \texttt{--reference\_pool}.}
    \label{fig:supp-fig2}
\end{figure}

By default, nf-core/viralmetagenome selects the cluster centroid as a scaffolding reference. To assess the reference influence, the pipeline’s code needed to be slightly modified to force NCBI genomes as scaffolding reference. This version of the code repository is available in the branch \url{https://github.com/Joon-Klaps/viralmetagenome/tree/force-external-ref-centroid}.
The pipeline was then ran with the following parameters: \texttt{--with\_umi}, \texttt{--umi\_deduplicate both}, \texttt{--skip\_read\_classification}, \texttt{--skip\_precluster}, \texttt{--intermediate\_consensus\_caller ivar}, \texttt{--consensus\_caller ivar} and other default parameters.
